...
public class YoutubeMixProvider implements YoutubeMixLoader {
    ...
    public AudioPlaylist load(
        ...
        return new BasicAudioPlaylist(playlistTitle, tracks, selectedTrack, false, mixUrl, tracks.get(0).getInfo().artwork);
    }
    ...
    private void extractPlaylistTracks(
        ...
        for (JsonBrowser video : browser.values()) {
            ...
            renderer.get("thumbnail").get("thumbnails").values().forEach(action -> thumbnails.add(action.get("url").text()));

            AudioTrackInfo trackInfo = new AudioTrackInfo(title, author, duration, identifier, false, uri, thumbnails.get(0));
