...
public class DefaultYoutubeTrackDetails implements YoutubeTrackDetails {
    ...
    private AudioTrackInfo loadTrackInfo() {
        ...
        videoDetails.get("thumbnail").get("thumbnails").values().forEach(action -> thumbnails.add(action.get("url").text()));

        return buildTrackInfo(videoId, videoDetails.get("title").text(), videoDetails.get("author").text(), temporalInfo, thumbnails);
    }
    ...
    private AudioTrackInfo loadLegacyTrackInfo() {
        ...
        List<String> thumbnails = new ArrayList<>();
        args.get("thumbnail").get("thumbnails").values().forEach(action -> thumbnails.add(action.get("url").text()));

        return buildTrackInfo(videoId, args.get("title").text(), args.get("author").text(), temporalInfo, thumbnails);
    }

    private AudioTrackInfo buildTrackInfo(String videoId, String title, String uploader, TemporalInfo temporalInfo, List<String> thumbnails) {
        return new AudioTrackInfo(title, uploader, temporalInfo.durationMillis, videoId, temporalInfo.isActiveStream,
                "https://www.youtube.com/watch?v=" + videoId, thumbnails.get(0));
    }
